...
3. **Опубликовать:**
   ...
   - Для `drive.readonly` проверка Google обычно не требуется

**Примечание:** Для `drive.readonly` scope обычно проверка не требуется, приложение может быть опубликовано сразу.
...
- **Проверка Google:** для `drive.readonly` обычно не требуется, но может занять несколько дней для других scopes
